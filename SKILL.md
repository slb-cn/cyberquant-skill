...
3. **后台执行导出（固定用 stream）** —— stream（SSE）是长连接拉取，耗时长，**必须在后台线程执行，不要阻塞当前对话**：
   - 用 **Bash 工具 + `run_in_background: true`** 启动以下命令，拿到后台任务后**不要等待其完成**（无需传页码，stream 自动流式拉取全部数据）：
     ```bash
     cyberquant-cli stream <routeSlug> \
       --format csv --output <路径> \
       --<参数1> <值1> --<参数2> <值2> ...
     ```
   - 启动成功后**立即回复用户**（不等导出跑完）：「数据正在后台导出中，你可以关注输出文件 `<绝对路径>` 的内容变化（随拉取逐步增长）；导出完成后我会再通知你。」
   - 后台任务结束时（harness 会推送完成通知）再继续第 5 步，向用户报告导出完成。
...
5. **后台任务完成后再报告**：当后台导出任务结束（harness 推送完成通知后，用 TaskOutput/Read 读取其输出摘要），**此时才向用户回复「导出完成」**：给出**输出文件的绝对路径 + 数据行数**（stream 完成后 CLI 会打印摘要）。若后台输出报「该路由不支持 stream / SSE」类错误，告知用户此路由暂不支持流式导出，可改用对话内分析。
...
3. **导出必须后台执行**：用 Bash `run_in_background: true` 启动 stream，**先回复「数据导出中」**（提示关注输出文件内容变化），**后台跑完再回复「导出完成」**（给路径 + 行数）——严禁阻塞对话、严禁同步等待。
4. 所有回复（数据/错误/警告）都附带清晰的下一步提示。
5. 第 0 步的安装 / 注册 / 配置必须**幂等**：已就绪就跳过，绝不重复安装或覆盖用户已有配置。
